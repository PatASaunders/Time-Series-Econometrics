...
qplot(x = Oil_Data$Date, y = Oil_Data$'Close', geom='line') + geom_line(color='dark blue') +
  labs(x='Year', y='Price ($)', title='Brent Closing Price') + geom_hline(yintercept = mean(Oil_Data$'Close'), color='red')

qplot(x = Gold_Data$Date, y = Gold_Data$'Close', geom='line') + geom_line(color='dark blue') +
  labs(x='Year', y='Price ($)', title='Gold Closing Price') + geom_hline(yintercept = mean(Gold_Data$'Close'), color='red')


dates <- as.POSIXct('2009-01-01','2010-12-31','2014-01-01','2016-12-31','2019-11-17', tz = "Europe/Paris")
DJI_Data$Date = as.POSIXct(DJI_Data$Date, tz = "Europe/Paris")
attr(dates,"tzone")


incr <- seq(8134,length.out=nrow(DJI_Data), by=-1)
DJI_Data <- cbind(DJI_Data, incr)
p3_1 = qplot(DJI_Data$Date , DJI_Data$`Adj Close`, geom = 'line') + labs(x=' ', y='DJI') +
       annotate("rect", xmin = as.Date('2009-04-01'), xmax = as.Date('2010-08-10'), ymin = -Inf, ymax = Inf, alpha = .2, fill='yellow') +
       annotate("rect", xmin = as.Date('2014-01-24'), xmax = as.Date('2016-06-5'), ymin = -Inf, ymax = Inf, alpha = .2, fill='blue') +
       annotate("rect", xmin = as.Date('2019-11-17'), xmax = as.Date('2020-4-10'), ymin = -Inf, ymax = Inf, alpha = .2, fill='red')
p3_2 = qplot(Oil_Data$Date, Oil_Data$Close, geom = 'line') + labs(x=' ', y='Brent Price') +
       annotate("rect", xmin = as.Date('2009-04-01'), xmax = as.Date('2010-08-10'), ymin = -Inf, ymax = Inf, alpha = .2, fill='yellow') +
       annotate("rect", xmin = as.Date('2014-01-24'), xmax = as.Date('2016-06-05'), ymin = -Inf, ymax = Inf, alpha = .2, fill='blue') +
       annotate("rect", xmin = as.Date('2019-11-17'), xmax = as.Date('2020-4-10'), ymin = -Inf, ymax = Inf, alpha = .2, fill='red')
p3_3 = qplot(Gold_Data$Date, Gold_Data$Close, geom = 'line') + labs(x='Date', y='Gold Price') +
       annotate("rect", xmin = as.Date('2009-04-01'), xmax = as.Date('2010-08-10'), ymin = -Inf, ymax = Inf, alpha = .2, fill='yellow') +
       annotate("rect", xmin = as.Date('2014-01-24'), xmax = as.Date('2016-06-5'), ymin = -Inf, ymax = Inf, alpha = .2, fill='blue') +
       annotate("rect", xmin = as.Date('2019-11-17'), xmax = as.Date('2020-4-10'), ymin = -Inf, ymax = Inf, alpha = .2, fill='red')

#DJI_Data$Date = as.Date(DJI_Data$Date, format = '%Y-%m-%d')

grid.newpage()
grid.draw(rbind(ggplotGrob(p3_1), ggplotGrob(p3_2), ggplotGrob(p3_3), size="last"))
